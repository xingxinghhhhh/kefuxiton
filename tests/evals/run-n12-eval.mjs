...
assert(cases.length >= 10, 'N12 eval must cover release, harness, rollback, and cleanup boundaries');
for (const category of ['isolated_schema', 'single_migration', 'published_fixture', 'production_denial', 'customer_smoke', 'test_harness', 'safe_summary', 'rollback', 'owned_cleanup', 'failure_codes']) {
...
assert(rehearsal.includes(`baselineCommit = process.env.REHEARSAL_BASELINE_COMMIT ?? '91942d9'`) && rehearsal.includes("git', ['diff', '--quiet'"), 'rollback must pin and verify the N11 baseline');
...
console.log(`N12 eval passed: ${cases.length} synthetic release rehearsal, production boundary, harness, rollback, and cleanup cases`);
